...
def new_tab(notebook, tab_type, tab = 'default', default_values = '', tabs_textbox = {}):
    ...
    if tab_type == "area":
        instructions = tk.Text(frame, width=82, height=20)
        instructions.pack()
        instructions.insert(tk.END, 'Make sure that you have the necessary files in the root directory of the program. The first time you run the program, you should have a file called "reference.csv" in the same location as the program. On subsequent runs, you should also have a file called "settings.json" in the root directory.')
    elif tab_type == "box":
        textboxes = []
